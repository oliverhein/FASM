...
public class ChartCreateThread extends Thread
{
   ...
   Vector trademengelist             = new Vector();
   ...
   public void run()
   {
    ...
    private void  processoneNetwork(String pDirName, String pNetworkName )
    {
      ...
      private boolean loadPrice_Innervalue_Umsatz(String pPriceFile)
      {
       ...
       try
       {
           ...
           do
           {
              ...
              dd =  HelpTool.TrimString( dd, '.');
              dd =  dd.replace(',','.');
              this.trademengelist.add( new Double( dd ) );

              dd =  ss.substring(j5+1, j6);
              dd =  HelpTool.TrimString( dd, '.');
              dd =  dd.replace(',','.');
      ...
      {
        ex.printStackTrace();
        JOptionPane.showMessageDialog(null, "Error while loading LogRenditen.","Error", JOptionPane.ERROR_MESSAGE);
        return false;
      }
